...
//    }
    ...
    public static Bitmap ORB2(Mat firstPath, Mat secondPath){
        ...
        for (int i = 0; i < obj_keypont.size(); i++){
            try {
                obj_pointsList.addLast(obj_keypont.get(matchesList.get(i).queryIdx).pt);
                scene_pointsList.addLast(scene_keypont.get(matchesList.get(i).trainIdx).pt);
        ...
        Features2d.drawMatches(firstPath, keypoints1, secondPath, keypoints2, matches, outputImg, GREEN, RED, new MatOfByte(), Features2d.NOT_DRAW_SINGLE_POINTS);
    ...
    private static LinkedList<KeyPoint> crossCheck(List<DMatch> matches12, List<DMatch> matches21, List<KeyPoint> keypoints,List<DMatch> filteredMatches){
        LinkedList<KeyPoint> filteredKeypoints = new LinkedList<KeyPoint>();
        ...
        for (int i = 0; i < matches12.size(); i++){
            ...
            if (backward.trainIdx ==  forward.queryIdx){
                ...
                filteredKeypoints.addLast(keypoints.get(i));
            }
        }
